...
import { InventorySearchResult } from "@/components/InvoiceItem";
...
const ItemsCard = ({
  ...
  invoice: Invoice;
  onAddItem: () => void;
  onUpdate: (id: string, updated: Partial<InvoiceItem>) => void;
  onDelete: (id: string) => void;
  onBulkUpload: () => void;
  onProductSelector: () => void;
    ...
    <CardHeader className="pb-3 pt-5 px-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={onProductSelector} size="sm" variant="outline">
            <Package className="w-4 h-4 mr-1" /> From Inventory
          </Button>
          <Button onClick={onBulkUpload} size="sm" variant="outline">
            <Upload className="w-4 h-4 mr-1" /> Bulk Upload
          </Button>
          <Button onClick={onAddItem} size="sm" className="bg-green-600 hover:bg-green-700 text-white">
    ...
    <CardContent className="px-5 pb-5 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          />
          {invoice.inclusiveTax && <p className="text-xs text-gray-400 mt-1">Disabled — tax included in item prices</p>}
        </div>
        <div>
          <Label className="text-xs text-gray-500 mb-1 block">Overall Discount</Label>
          <div className="flex gap-1">
            <Select value={invoice.discountType} onValueChange={(v: "fixed" | "percentage") => setInvoice((p) => ({ ...p, discountType: v, discountValue: 0 }))}>
      ...
      {invoice.items.some((i) => (i.itemDiscountAmount ?? 0) > 0) && (
        <div className="bg-red-50 border border-red-100 rounded-lg px-3 py-2 space-y-1">
          ...
          {invoice.items.filter((i) => (i.itemDiscountAmount ?? 0) > 0).map((i) => (
            <div key={i.id} className="flex justify-between text-xs text-red-500">
              ...
              <span>−₹{(i.itemDiscountAmount ?? 0).toFixed(2)}{i.itemDiscountType === "percentage" ? ` (${i.itemDiscountValue}%)` : ""}</span>
            </div>
          ))}
        </div>
      )}
      <div className="border-t pt-4 space-y-2 text-sm">
        ...
        <div className="flex justify-between text-gray-600">
          <span className="flex items-center gap-1">
            ...
            {invoice.inclusiveTax && <span className="text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded">Included</span>}
          </span>
          <span>{invoice.inclusiveTax ? "—" : `₹${invoice.taxAmount.toFixed(2)}`}</span>
        </div>
        {invoice.discountAmount > 0 && (
          <div className="flex justify-between text-red-500">
            <span>Overall Discount {invoice.discountType === "percentage" ? `(${invoice.discountValue}%)` : "(fixed)"}</span>
...
const AddCustomerDialog = ({
  ...
  newCustomer: { name: string; email: string; address: string; phone: string };
  ...
  <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent>
      ...
      <div className="space-y-4">
        ...
        <div><Label>Address</Label><Textarea value={newCustomer.address} onChange={(e) => setNewCustomer({ ...newCustomer, address: e.target.value })} /></div>
        <div className="flex justify-end gap-2">
          <Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
          <Button onClick={onSave} disabled={isSaving}>{isSaving ? "Saving..." : "Add Customer"}</Button>
        </div>
      </div>
    </DialogContent>
  </Dialog>
...
const Invoices = () => {
  ...
  const [layoutMode, setLayoutMode] = useState<"original" | "new">("new");
  ...
  const fetchBusinessSettings = async () => {
    if (!ownerId) return;
    try {
      const { data, error } = await (supabase as any).from("business_settings").select("*").eq("user_id", ownerId).single();
  ...
  const fetchBusinessName = async () => {
    if (!ownerId) return;
    try {
      const { data, error } = await supabase.from("business_settings").select("business_name").eq("user_id", ownerId).single();
      if (data && !error) setBusinessName(data.business_name);
    } catch { console.error("Error fetching business name"); }
  };

  // ── Inventory search for autocomplete ─────────────────
  const searchInventory = async (query: string): Promise<InventorySearchResult[]> => {
  ...
  const addItem = () => {
    const newItem: InvoiceItem = {
      ...
      date: new Date().toISOString().split("T")[0],
      orderId: "", description: "", quantity: 1, unitPrice: 0, amount: 0,
      hsnCode: "", itemTaxRate: 0, itemDiscountType: "flat",
      itemDiscountValue: 0, itemDiscountAmount: 0, itemTaxAmount: 0,
    };
  ...
  const handleProductSelect = (product: InventoryProduct, quantity: number, variant?: any) => {
    ...
    const newItem: InvoiceItem = {
      id: Date.now().toString(),
      date: new Date().toISOString().split("T")[0],
      orderId: product.sku || "", description, quantity, unitPrice,
      amount: quantity * unitPrice, hsnCode: variant?.hsn_code ?? "",
      itemTaxRate: 0, itemDiscountType: "flat", itemDiscountValue: 0,
  ...
  const topBar = (
    <div className={`flex items-center justify-between mb-8 ${layoutMode === "new" ? "bg-white border-b sticky top-0 z-10 shadow-sm px-6 py-3 -mx-6" : ""}`}>
      <div className="flex items-center gap-4">
        <Button onClick={() => navigate("/")} variant={layoutMode === "new" ? "ghost" : "outline"} size="sm">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center bg-gray-100 rounded-lg p-1 border border-gray-200 gap-1">
          <button onClick={() => setLayoutMode("original")} title="Side-by-side"
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${layoutMode === "original" ? "bg-white shadow-sm text-gray-900 border border-gray-200" : "text-gray-500 hover:text-gray-700"}`}>
            <LayoutPanelLeft className="w-3.5 h-3.5" /> Side
          </button>
          <button onClick={() => setLayoutMode("new")} title="Stacked"
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${layoutMode === "new" ? "bg-white shadow-sm text-gray-900 border border-gray-200" : "text-gray-500 hover:text-gray-700"}`}>
        ...
        <Button onClick={exportToPDF} size={layoutMode === "new" ? "sm" : "default"}
  ...
  const itemsCardProps = {
    ...
    onProductSelector: () => setShowProductSelector(true),
    onInventorySearch: searchInventory,
  };
  ...
  return (
    <div className={`min-h-screen ${layoutMode === "original" ? "bg-gradient-to-br from-green-50 via-white to-yellow-50" : "bg-gray-50"}`}>
      ...
      <div className={`${layoutMode === "new" ? "max-w-5xl mx-auto px-6 py-6" : "container mx-auto px-4 py-8"}`}>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input type="number" value={invoice.taxRate} disabled={invoice.inclusiveTax}
                        onChange={(e) => setInvoice((p) => ({ ...p, taxRate: parseFloat(e.target.value) || 0 }))} />
                  ...
                  <div className="mt-4">
                    ...
                    <Input type="number" value={invoice.discountValue}
                      onChange={(e) => setInvoice((p) => ({ ...p, discountValue: parseFloat(e.target.value) || 0 }))} />
                  </div>
                  <div className="border-t mt-4 pt-4 space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span>{invoice.inclusiveTax ? <span className="text-gray-400">—</span> : <>₹{invoice.taxAmount.toFixed(2)}</>}</span>
                    </div>
                    {invoice.discountAmount > 0 && (
                      <div className="flex justify-between text-red-600"><span>Discount:</span><span>-₹{invoice.discountAmount.toFixed(2)}</span></div>
                    )}
        ...
        {layoutMode === "new" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <NotesCard invoice={invoice} setInvoice={setInvoice} />
              <TotalsCard invoice={invoice} setInvoice={setInvoice} />
            </div>
            {showPreview && (
              <div>
      ...
      <ProductSelector open={showProductSelector} onOpenChange={setShowProductSelector}
